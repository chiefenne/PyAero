...
from PyAero import __version__, __appname__, __author__
...
class Windtunnel:
    ...
    def makeMesh(self):
        ...
        if self.foil.spline_data is None:
            message = 'Splining needs to be done first.'
            logger.debug(message)
    #     # delete blocks if existing
    #     # because a new one will be generated
        if len(self.blocks)!=0:
            self.blocks = []

        contour = self.foil.spline_data[0]

        # mesh around airfoil
        self.AirfoilMesh(name='block_airfoil', contour=contour)
    ...
    def AirfoilMesh(self, name='', contour=None):
        acm = self.mesh_parameters['Airfoil contour mesh']
    ...
    def TrailingEdgeMesh(self, name='', te_divisions=3,
        ...
        tem = self.mesh_parameters['Airfoil trailing edge mesh']
    ...
    def TunnelMesh(self, name=''):
        tam = self.mesh_parameters['Windtunnel mesh airfoil']
    ...
    def TunnelMeshWake(self, name='', tunnel_wake=2.0,
        ...
        twm = self.mesh_parameters['Windtunnel mesh wake']
    ...
    def makeBoundaries(self):
        ...
        # FIXME
        maxX = np.max(np.array(vertices)[:,0])
    ...
    def drawMesh(self):
        ...
        # if hasattr(self.airfoil, 'mesh'):
        #     logger.debug('MESH item type: {}'.format(type(self.airfoil.mesh)))
        #     self.mainwindow.scene.removeItem(self.airfoil.mesh)
        ...
        for block in self.blocks:
            for lines in [block.getULines(),
                ...
                for line in lines:
                    ...
                    plt.plot(x_set, y_set, color='black', linewidth=1)
        plt.xlim([-0.2, 1.2])
        plt.ylim([-0.2, 0.2])
        plt.show()
...
class BlockMesh:
    ...
    def writeGMSH_OLD(wind_tunnel, name=''):
        ...
        with open(name, 'w') as f:
            ...
            f.write(' File created with ' + __appname__ + '\n')
